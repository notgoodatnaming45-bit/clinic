...
import { useEffect, useState } from "react";
import { getPatients, getPatientDocuments, getDocumentDownloadUrl } from "@/lib/api";
...
export default function AIEnginePage() {
  ...
  async function runAnalysis() {
    if (!selectedDocumentId) { alert("Select a document first."); return; }
    setLoading(true);
    try {
      const token = localStorage.getItem("tbi_access_token") || "dev-token";
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000";
      const route = analysisType === "clinical" ? "analyze-clinical" : "analyze-legal";
      const response = await fetch(
        `${apiUrl}/api/v1/documents/${selectedDocumentId}/${route}`,
      ...
      {result && (
        <div className="rounded-2xl border bg-white p-4 shadow-sm sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            ...
            <span className="rounded-full bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700">
              {analysisType === "clinical" ? "Clinical Summary" : "Legal Analysis"}
            </span>
          </div>
          <div className="mt-4 overflow-auto rounded-2xl bg-slate-950 p-4 text-sm text-green-400 sm:mt-6 sm:p-5">
