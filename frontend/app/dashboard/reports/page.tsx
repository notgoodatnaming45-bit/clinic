...
export default function ReportsPage() {
  ...
  async function loadPatients() { const data = await getPatients(); setPatients(data); }
  async function loadReports(patientId: string) {
    if (!patientId) { setReports([]); return; }
    const data = await getPatientReports(patientId); setReports(data);
  }
  async function openReport(reportId: string) { const data = await getReport(reportId); setSelectedReport(data); }

  useEffect(() => { loadPatients(); }, []);
  useEffect(() => { setSelectedReport(null); loadReports(selectedPatientId); }, [selectedPatientId]);
  ...
  return (
    <div className="space-y-6 print:p-0">
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm print:hidden sm:p-6">
        <label className="mb-2 block text-sm font-medium text-slate-600">
          Select Patient
        </label>
        <select
      ...
      <div className="grid gap-6 lg:grid-cols-3 print:block">
        {/* Report list */}
        <div className="rounded-2xl border bg-white p-4 shadow-sm print:hidden sm:p-6 lg:col-span-1">
          <h2 className="text-lg font-bold text-slate-900 sm:text-xl">Report List</h2>
        ...
        {/* Report viewer */}
        <div className="rounded-2xl border bg-white p-4 shadow-sm print:border-0 print:shadow-none sm:p-8 lg:col-span-2">
          {!selectedReport ? (
            ...
            <div>
              <div className="mb-5 flex flex-col gap-3 print:block sm:mb-6 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                <div>
                  ...
                  {selectedPatient && (
                    <p className="mt-1 text-sm text-slate-600">
                      Patient: {selectedPatient.first_name_encrypted} {selectedPatient.last_name_encrypted} • MRN: {selectedPatient.mrn}
                    </p>
                  )}
                  ...
                  {selectedReport.approved_at && (
                    <p className="mt-1 text-xs text-slate-500">
                      Approved: {new Date(selectedReport.approved_at).toLocaleString()}
                    </p>
                  )}
                  {selectedReport.digital_signature && (
                    <p className="mt-1 break-all text-xs text-slate-500">
                ...
                <button
                  ...
                  className="w-full rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white hover:bg-blue-700 print:hidden sm:w-auto"
                >
                  Print / Save PDF
                </button>
              </div>
              <pre className="whitespace-pre-wrap break-words rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-800 print:bg-white print:p-0 sm:p-5">
                {reportContent}
              </pre>
            </div>
          )}
